...
import { deleteFamulusPrice } from "../actions/deleteFamulusPrice";
import { saveFamulusPrice } from "../actions/saveFamulusPrice";

const FamulusAdminPricesPage = () => {
    const { famulusPrices } = useSelector((state) => state.adminPrices);
    const [search, setSearch] = useState("");
    const [editing, setEditing] = useState<null | FamulusPrice | false>(false);

    const filtered = famulusPrices.filter((r) => r.name.toLowerCase().includes(search.toLowerCase()));

    const handleDelete = async (id: number) => {
        await deleteFamulusPrice(id);
    };

    const handleSave = async () => {
        if (editing === false) return;
        const isNew = !(editing as FamulusPrice)?.id;

        const payload = {
            ...(editing !== null && { id: editing.id }),
            name: editing?.name,
            default: editing?.default,
            weekend: editing?.weekend,
            external: editing?.external,
            external_weekend: editing?.external_weekend,
        };
        await saveFamulusPrice(payload, isNew);

        setEditing(false);
    };

    const handleInput = (_id: number, key: string, value: string) => {
        setEditing((prev) => (prev !== false ? { ...(prev as FamulusPrice), [key]: value } : prev));
    };

    const columns: ColumnsType<FamulusPrice> = [
        {
            title: "Szolgáltatás",
            dataIndex: "name",
            key: "name",
        ...
        ...PRICEVARIANTS.map(
            (variant) =>
                ({
                    title: variant.label,
                    dataIndex: variant.key,
                    key: variant.key,
                    width: "15%",
                    render: (value: number) => <span className="text-[14px] font-semibold text-[#3e484c]">{value}</span>,
                }) satisfies ColumnsType<FamulusPrice>[number]
        ),
        {
            title: "Műveletek",
            key: "actions",
    ...
    return (
        <div className="flex w-full flex-col gap-4">
            <Modal
                title={editing === null ? "Szolgáltatás hozzáadása" : "Szolgáltatás szerkesztése"}
                open={editing !== false}
                ...
                <div className="flex max-h-[70vh] w-full flex-col gap-4">
                    <div>
                        ...
                        <input
                            ...
                            value={(editing as FamulusPrice)?.name ?? ""}
                    ...
                    {PRICEVARIANTS.map((c, idx) => (
                        <div key={idx}>
                            ...
                            <input
                                ...
                                name={c.key}
                                placeholder="123..."
                                value={(editing as FamulusPrice)?.[c.key as keyof FamulusPrice] ?? ""}
            ...
            <div className="flex w-full flex-col items-center justify-between gap-4 min-[800px]:flex-row">
                <div className="text-[#3e484c]">
                    <h1 className="flex-1 text-2xl font-bold">Famulus Árlista</h1>
...
export default FamulusAdminPricesPage;
